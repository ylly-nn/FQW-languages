<script setup>
  ...
  onMounted(() => {
    ...
    if (stored) completedIds.value = new Set(JSON.parse(stored))

    const storedPremium = localStorage.getItem('vocabulary_premium')
    if (storedPremium) premiumIds.value = new Set(JSON.parse(storedPremium))
  })

  const saveCompleted = () => {
    localStorage.setItem('vocabulary_completed', JSON.stringify([...completedIds.value]))
  }
  const savePremium = () => {
    localStorage.setItem('vocabulary_premium', JSON.stringify([...premiumIds.value]))
  }

  const selectLesson = (lesson) => {
    // Позже можно сделать страницу урока
    router.push(`/vocabulary/lesson/${lesson.id}`)
  }
  ...
  const togglePremium = (lesson) => {
    ...
    premiumIds.value = new Set(premiumIds.value)
    savePremium()
  }
</script>
...
<template>
  <div class="root">
    <div class="top-bar">
      <BackButton to="/vocabulary/topics" label="← Темы" />
    </div>
