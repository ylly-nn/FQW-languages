...
func (h *Handler) Register(w http.ResponseWriter, r *http.Request) {
	log.Println("=== START /auth/register ===")
	...
	log.Printf("Request: email=%s, password length=%d", req.Email, len(req.Password))
	...
	log.Println("Calling authManager.Register...")
	err := h.auth.Register(req.Email, req.Password)
	if err != nil {
		log.Printf("❌ authManager.Register returned error: %v", err)
		switch err.Error() {
		case ErrUserAlreadyExists:
			http.Error(w, err.Error(), http.StatusConflict)
		default:
			// НЕ скрываем реальную ошибку в режиме отладки
			http.Error(w, "Internal server error: "+err.Error(), http.StatusInternalServerError)
		}
		return
	}

	log.Println("✅ Registration successful, sending response")
